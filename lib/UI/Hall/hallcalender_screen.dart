...
class HallCalendarScreen extends StatelessWidget {
  const HallCalendarScreen({super.key});

  @override
  Widget build(BuildContext context) {
    return BlocProvider(
      create: (_) => DemoBloc(),
      child: const HallCalendarScreenView(),
    );
  }
}

class HallCalendarScreenView extends StatefulWidget {
  const HallCalendarScreenView({super.key});

  @override
  State<HallCalendarScreenView> createState() => _HallCalendarScreenViewState();
}

class _HallCalendarScreenViewState extends State<HallCalendarScreenView> {
      ...
      children: ['Sun', 'Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat'].map((day) {
        return Expanded(
          child: Center(
            child: Text(
              day,
              style: MyTextStyle.f14(Colors.grey[700]!, weight: FontWeight.bold),
            ),
          ),
        );
      }).toList(),
        ...
        return GestureDetector(
          ...
          child: Container(
            decoration: isSelected
                ? BoxDecoration(color: appPrimaryColor, borderRadius: BorderRadius.circular(12))
                : null,
            child: Center(
              child: Text(
                '$day',
  ...
  Widget _buildSelectedDateInfo() {
    return Container(
      ...
      child: Column(
        children: [
          Text(
            'Selected Date for Hall Booking:',
        ...
        child: Padding(
          ...
          child: Column(
            ...
            children: [
              ...
              const SizedBox(height: 16),
              Text('Confirm Hall Booking', style: MyTextStyle.f18(appPrimaryColor, weight: FontWeight.bold)),
              const SizedBox(height: 16),
              Text('You are booking Hall for:', style: MyTextStyle.f14(Colors.grey[700]!)),
              const SizedBox(height: 8),
              Text(
                DateFormat('MMMM d, yyyy').format(_selectedDate!),
                style: MyTextStyle.f16(Colors.black, weight: FontWeight.bold),
              ),
                    ...
                    child: ElevatedButton(
                      ...
                      onPressed: () {
                        ...
                        Navigator.push(
                          context,
                          MaterialPageRoute(
                            builder: (context) => HallBookingScreen(selectedDate: _selectedDate!),
                          ),
                        );
                      },
